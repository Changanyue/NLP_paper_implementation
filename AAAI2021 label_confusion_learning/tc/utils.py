        ...
        def load_dataset(path, pad_size=32):
            ...
            with open(path, 'r', encoding='UTF-8') as f:
                for line in tqdm(f):
                    ...
                    if not lin:
                        continue
                    content, label_true = lin.split('\t')
                    token = config.tokenizer.tokenize(content)
                    token = [CLS] + token
                    seq_len = len(token)
                    mask = []
                    token_ids = config.tokenizer.convert_tokens_to_ids(token)
                    label_train = [np.array(range(config.num_classes))]
                    ...
                    if pad_size:
                        if len(token) < pad_size:
                            ...
                            seq_len = pad_size
                    contents.append((token_ids, int(label_true), label_train,seq_len, mask))
            return contents
